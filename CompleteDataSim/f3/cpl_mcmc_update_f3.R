...
# initial values are not close to their true values
...
for (m in 1:n_iter){  
  ...
  ##sample v
  tilting_v=BigVX_sim%*%v+rep(e,T)
  omega_v=rpg(num=sum(T), h=1, z=tilting_v)      ## sample w_{it}^{*}
  k_v=Y_sim-omega_v*rep(e, T)-1/2
  S_v=solve((1/v_pri)*diag(2)+crossprod(BigVX_sim,diag(omega_v))%*%BigVX_sim )
  m_v=S_v%*%crossprod(BigVX_sim,k_v)
  v=as.vector(rmvnorm(n=1, mean=m_v, sigma=S_v))
  
  ##sample e_{i}
  for(i in 1:n){
    e_index=c(rep(0,i-1),1,rep(0, n-i))
    pos=which(rep(e_index, T)==1)
    S_e[i]=((1/E)+sum(omega_v[pos]))^{-1}
    m_e[i]=S_e[i]*sum(Y_sim[pos]-1/2-omega_v[pos]*(BigVX_sim[pos, ]%*%v))
  }
