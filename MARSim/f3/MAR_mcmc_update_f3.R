#---------------------------------------------------
# This script is used to update mcmc in MAR setting
# Last updated date: 8/20/2017
#---------------------------------------------------
source("/Users/peterwang/Dropbox/missingdata/Project/code/MARSim/f3/MAR_data_generation_f3.R")
...
inits=list(eta=c(0, 2, 13, 7), 
           ...
           sgm2=10, 
           E=10, 
           b=rnorm(n, mean=0, sd=sqrt(10)),             ##initial value of random effect b_{i}
           e=rnorm(n, mean=0, sd=sqrt(10))              ##initial value of random effect e_{i}
)

#initial missing value of X_{it}: linear interpolation
#Input data needs at least 2 non-NA data point for applying na.interpolation
#so if there is only 1 non-NA for i-th subject, we impute other missing X_{it} using this non-NA. 
for(i in 1:n){
  pos=which(datfrm$Subject==i)
  if (sum(R_sim[pos]==1)>1 ){
    X_sim[pos]=na.interpolation(X_sim[pos], option = "linear")
  } else {
    nonNA=(X_sim[pos])[R_sim[pos]==1]
    X_sim[pos]=rep(nonNA, T[i])
  }
}

#evaluate the above linear interpolation method 
#(max.abs.reladiff.ini.X=max(abs(datfrm$X-X_sim)/(datfrm$X)))
#(mean.abs.reladiff.ini.X=mean(abs(datfrm$X-X_sim)/(datfrm$X)))
...
for (m in 1:n_iter){  
  ...
  ##impute missing data using M-H algorithm
  Bigold=((exp(BigVX[R_sim==0, ]%*%v+rep(e,T)[R_sim==0]))^(Y_sim[R_sim==0]))/(1+exp(BigVX[R_sim==0, ]%*%v+rep(e,T)[R_sim==0]))
  ##proposal of missing value
  Pro.X=X
  Pro.X[R_sim==0]=D[R_sim==0, ]%*%beta+D_star[R_sim==0]*BigM[R_sim==0]+D_dstar[R_sim==0]*Bigb[R_sim==0]+
