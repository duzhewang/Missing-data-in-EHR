...
#initial missing value of X: randomly sample missing X_{it} from subject i's available observations with replacement. 
for (i in 1:n){
  len=length(X_sim[SVXYR$Subject==i & SVXYR$R==0])
  X_sim[SVXYR$Subject==i & SVXYR$R==0]=sample(X_sim[SVXYR$Subject==i & SVXYR$R==1], size = len, replace = TRUE )
}
